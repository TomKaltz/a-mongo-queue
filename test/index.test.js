...
test('fail if we do not provide a db connection', async t => {
  ...
  t.is(err.message, 'a-mongo-queue: must provide a mongodb.MongoClient')
})

test('fail if we do not provide a queue name', async t => {
  let err = await t.throws(mongoDbQueue(db))
  t.is(err.message, 'a-mongo-queue: must provide a queue name')
})

test('constructor returns a promise', t => {
...
test('can create a valid queue object', async t => {
  ...
  t.is(typeof queue.ensureIndexes, 'function')
...
test('enqueue and then get a message', async t => {
  ...
  t.not(msg, undefined, 'previous statement should have returned a message')
