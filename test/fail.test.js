...
test('returns a promise', t => {
  let res = q.fail('doesnotexist')
  res.catch(()=>{})
  t.is(res.constructor.name, 'Promise', 'returns a promise object')
  t.is(typeof res.then, 'function', 'returns a thanable')
})

test('rejects promise if the id is not failable', async t => {
  await t.throws(q.ack('thisiddoesnotexist'))
})

test('add a message, dequeue it, then fail it, resolving same message id', async t => {
  await q.enqueue()
  let msg = await q.dequeue()
  t.deepEqual(msg.id, await q.fail(msg.id))
})

test('reject fail on an expired lock', async t => {
  await q.enqueue()
  let msg = await q.dequeue({lockDuration: 1})
  MockDate.set(Date.now() + 1100)
  await t.throws(q.fail(msg.id))
  MockDate.reset()
})

test('reject fail if the message was already acked', async t => {
  await q.enqueue()
  let msg = await q.dequeue()
  t.truthy(msg) 
